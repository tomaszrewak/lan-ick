"""Experiment runner: Position-aware top-N feature selection.

Sweep N values → select features → train OVR → evaluate.
Also evaluates per-type thresholds (FP budget ≤5%).
"""
...
def main():
    ...
    for top_n in TOP_N_VALUES:
        ...
        for et in ErrorType:
            if et not in classifier.models:
                continue
            # Find lowest threshold where FP count ≤ budget
            best_t = 1.0
